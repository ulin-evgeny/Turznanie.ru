 ...
 *
 * ВНИМАНИЕ! Не надо использовать этот класс для валидации сохраняемых в БД значений. Для этого есть ORM (в нем свой метод check()). Записываем в модель значение, а потом у модели вызываем check().
 * Просто потому что в этом классе нельзя получить максимальную длину поля в БД - как это можно сделать в ORM:
 * $this->_table_columns[$column]['character_maximum_length']
 * Также здесь нет фильтров (и HTMLPurifier'а в них).
 *
 * "...для валидации сохраняемых в БД значений." Что я имею в виду?
 * Я имею в виду, когда сначала идет валидация разных полей у разных связанных моделей. А потом, если валидация пройдена, то поочередное сохранение объектов этих моделей.
 * Так вот. Если делать валидацию через этот класс. То при сохранении (методы save() или update() у ORM) автоматически будет вызываться метод check() из класса ORM. Может произойти такая ситуация, когда валидация через этот класс пройдена. Часть сязанных моделей сохранилась в БД. А потом у одной модели "проваливается" check() от ORM. Пользователю выдается сообщение о том, что валидация не пройдена. Что ему нужно изменить его ввод. Однако часть моделей уже сохранилась в БД. И это плохо в БД появляется мусор.
 *
 * @package    Kohana
 * @category   Security
 * @author     Kohana Team
...
class Kohana_Validation implements ArrayAccess {
	...
	const DEFAULT_ERRORS_FILE = 'validation';
	...
	public function check_with_captcha() {
		$validation = $this;
		if (!$validation->check()) {
			// В отличие от метода check в ORM, здесь исключение не выбрасывается, поэтому мы выбрасываем его сами. Это нужно, чтобы сократить код для отправки его клиенту - вместо if и try будет только try.
	 ...
	 * @return  array
	 */
	public function errors($file = NULL, $translate = TRUE, $default_errors = true) {
		if ($file === NULL) {
		...
		foreach ($this->_errors as $field => $set) {
			...
			$label = isset($this->_labels[$field]) ? $this->_labels[$field] : null;
			...
			if (!is_string($message)) {
				if ($default_errors) {
					$file = static::DEFAULT_ERRORS_FILE;
					if ($message = Kohana::message($file, "{$field}.{$error}") AND is_string($message)) {
						// Found a message for this field and error
					} elseif ($message = Kohana::message($file, "{$field}.default") AND is_string($message)) {
						// Found a default message for this field
			...
			while (preg_match('/(\$\$[0-9]+,[a-zA-Zа-яА-ЯёЁ0-9_-]*,[a-zA-Zа-яА-ЯёЁ0-9_-]*,[a-zA-Zа-яА-ЯёЁ0-9_-]*\$\$)/', $message, $string)) {
				$string = reset($string);
				$to_explode = str_replace('$', '', $string);
				$exploded = explode(',', $to_explode);
				$result = Helper::form_of_word($exploded[0], $exploded[1], $exploded[2], $exploded[3]);
				$message = str_replace($string, $result, $message);
			}
