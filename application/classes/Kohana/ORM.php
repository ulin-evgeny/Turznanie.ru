...
class Kohana_ORM extends Model implements serializable {
	...
	public function reset_and_count() {
		$this->reset(false);
	...
	protected function _validation() {
		/* Я несколько изменил этот метод - теперь при update он проверяет только измененные поля, а не все. Это и быстрее, и надежнее - после валидации можно как-нибудь изменить строку (например, применить HTMLPurifier) и только потом сохранить в БД. Потом, если меняем какое либо поле у модели (например, количество входов - login), то наша строка НЕ ПРОВЕРЯЕТСЯ (и не вылетают ошибки валидации - что она длиннее, чем нужно, так как ее обработал HTMLPurifier).  */
		...
		foreach ($this->rules() as $field => $rules) {
			// если _loaded == true, значит, это обновление строки, а не создание новой. А значит, валидируем только те значения, которые были изменены.
			if ($this->_loaded) {
	...
	public function set($column, $value) {
		...
		if (array_key_exists($column, $this->_object)) {
			...
			// HTMLPurifier
			// -------------------------------
			// проверка is_null нужна, так как HTMLPurifier null превращает в "" (пустую строку).
			if (!is_null($value)) {
				$purifier = HelperHTMLPurifier::get_purifier();
				if ((!in_array($column, WHITE_LIST_COLUMNS)) && isset($this->_table_columns[$column]['character_maximum_length'])) {
	...
	public function check_with_captcha(Validation $extra_validation = NULL) {
		$this->check($extra_validation);
		$validation = new Validation($_POST);
		$validation = HelperReCaptcha::add_rule($validation);
		$validation->check();
	}
	...
	public function check(Validation $extra_validation = NULL) {
		...
		$extra_errors = ($extra_validation AND (count($extra_validation->errors()) || !$extra_validation->check()));
		...
		// ---------------------------------------------
		// Стандартная валидация - проверка rules
		// ---------------------------------------------
		...
		// ---------------------------------------------
		// Определение текстовых и числовых полей для проверки на максимальную длину / значение
		// ---------------------------------------------
		foreach ($array->data() as $column => $value) {
			if (isset($this->_table_columns[$column]['character_maximum_length'])) {
				$text_columns[$column] = $value;
			}
			if (isset($this->_table_columns[$column]['max']) && isset($this->_table_columns[$column]['min'])) {
				$num_columns[$column] = $value;
			}
		}

		// ---------------------------------------------
		// Проверка текстовых полей на максимальную длину
		// ---------------------------------------------
		if (!empty($text_columns)) {
			$text_validation = new Validation($text_columns);
			$text_validation->labels($this->labels());
			foreach ($text_validation->data() as $key => $val) {
				$text_validation->rule($key, 'max_length', array(':value', $this->_table_columns[$key]['character_maximum_length']));
			}
			if (!$text_validation->check()) {
				throw new ORM_Validation_Exception(null, $text_validation);
			}
		}

		// ---------------------------------------------
		// Проверка числовых полей на максимальное значение
		// ---------------------------------------------
		if (!empty($num_columns)) {
			$num_validation = new Validation($num_columns);
			$num_validation->labels($this->labels());
			foreach ($num_validation->data() as $key => $val) {
				$num_validation->rule($key, 'range', array(':value', $this->_table_columns[$key]['min'], $this->_table_columns[$key]['max']));
			}
			if (!$num_validation->check()) {
				throw new ORM_Validation_Exception(null, $num_validation);
			}
		}
		// ---------------------------------------------
	...
	public function delete() {
		if (!$this->_loaded) {
			throw new Kohana_Exception(Messages::DB_ITEM_NOT_FOUND);
		}
